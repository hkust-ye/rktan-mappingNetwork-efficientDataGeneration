...
def stress_and_compliance(x0,fx0,fy0):
    ...
    #s=1/E0*np.array([[1,-poi,0],[-poi,1,0],[0,0,2+2*poi]])
    flag=1
    ...
    for test_case in range(1):
        ...
        #K11=TK[degL.reshape(-1,1),degL]
        #K12=TK[degL.reshape(-1,1),degO]
        ...
        #TF=TK*TU
    #compliance=np.dot(np.dot(TU.T,K.toarray()),TU)
    Xigem_e=np.zeros((nelx*nely,3))
    #C_sen=np.zeros(nelx*nely)
    ...
    for num in range(nelx*nely):
        ...
        Xigem_e[num,:]=1/(dx)*np.dot(np.dot(CP,NE),Ue).T*(Emin+x_temp[0,num]*(E0-Emin))/E0 #1/(dx*dy)*np.dot(np.dot(CP,NE),Ue).T*(Emin+x_temp[0,num]*(E0-Emin))/E0
        #C_sen[num]=-ps*(Emin+x_temp[0,num]*(E0-Emin))*np.dot(np.dot(Ue.T,KE),Ue)
        #C_sen[num]=-ps*((x_temp[0,num]**(ps-1))*(E0-Emin))*np.dot(np.dot(Ue.T,KE),Ue)
        ...
        #print(Xigem_e[num,:].shape)
    print('cmp=',compliance)
    return Xigem_e,compliance,compliance_e


def filtering(f,H):
    Hs = sparse.csr_matrix.sum(H,axis=1)
    filtered_value=sparse.csr_matrix.dot(H,f)/Hs
...
def comp_pred(xphy,fy0,nelx,nely):
    ...
    predicted_comp = model.predict(x_data)
    comp_tmp = np.maximum(predicted_comp*1e-5,0)
    comp_output = comp_tmp.reshape(nelx,nely).T

    print('cmp=',np.sum(comp_output))

    return np.sum(comp_output),comp_output.reshape(nelx*nely)

def optimization(nelx,nely,fx0,fy0,Vol0,case_num):#(x_ini,fx0,fy0,Vol0):
    iter_num=100
    p=4
    error1=1
    error2=1
        ...
        for j1 in range(nely):
            ...
            for i2 in range(int(max((i1-np.fix(rmin)),0)),int(min(i1+np.fix(rmin)+1,nelx))):
                for j2 in range(int(max((j1-np.fix(rmin)),0)),int(min(j1+np.fix(rmin)+1,nely))):
                    ...
                    k = k+1
    H = sparse.csr_matrix((sH.flatten(),(iH.flatten().astype(int),jH.flatten().astype(int))))
    sparse.save_npz('canti6H128.npz',H)
    # H = sparse.load_npz('canti4H128.npy')
    
    x_ini=np.ones((eleNum,1))   
    x_temp=filtering(x_ini,H) 
    Cer=0.025
    Vol=np.sum(x_temp)
    ...
    compliance, compliance_e = comp_pred(xphy.reshape(nelx,nely).T,fy0,nelx,nely)

    itr=0
    Cmp[itr,0]=compliance

    fi0=open('opt_result/dens'+str(case_num)+'_'+str(Vol0)+'.csv','ab')
    np.savetxt(fi0,xphy.reshape(1,nelx*nely),fmt='%1d',delimiter=',')
    fi0.close()

    fi2=open('opt_result/fy'+str(case_num)+'_'+str(Vol0)+'.csv','ab')
    np.savetxt(fi2,fy0.reshape(1,nelx*nely),fmt='%e',delimiter=',')
    fi2.close()

    fi3=open('opt_result/comp'+str(case_num)+'_'+str(Vol0)+'.csv','ab')
    ...
    for itr in range(iter_num-1):
          ...
          Pd_filtered[itr,:]=-filtering(xphy*((compliance_e/compliance).reshape(-1,1)),H).reshape(-1,)
          if itr==0:
          ...
          flagA=flag
          while Car>Car_max and flagA<nelx*nely-1: # to determine BTA
                flagA=flagA+1
          ...
          Volt=np.sum(x_temp)
          flagD=flag
          while Volt<VolC and flagD>1: # to determine BTA
                flagD=flagD-1
...
for i in range(6):
    for vo in range(vol.size):
        ...
        fi1=open('opt_result/dens_final.csv','ab')
        np.savetxt(fi1,d_m.reshape(1,nelx*nely),fmt='%1d',delimiter=',')
        fi1.close()

        fi3=open('opt_result/comp_final.csv','ab')
